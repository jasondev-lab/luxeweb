...
class WebdesignController extends Controller
{
    ...
    public function index()
    {
        ...
        $web['description']=Setting::where('meta_key', 'web-design-description')->first();
        ...
        $setting_web_images=Setting::where('meta_key', 'web-design-images')->first();
        $web_images=$setting_web_images['meta_value'];
        ...
        for($i=0; $i<count($web_images); $i++){
            $thumb_name=$image_sizes['web_image_width'].'_'.$image_sizes['web_image_height'].'_'.$web_images[$i]['name'];
            ...
            if(!File::exists(public_path('uploads/home/thumb').'/'.$thumb_name)){
                $thumb = Image::make(public_path('uploads/home').'/'.$web_images[$i]['name']);
                $thumb->resize($image_sizes['web_image_width'], $image_sizes['web_image_height'], function ($const) {
                    $const->aspectRatio();
                })->save(public_path('uploads/home/thumb').'/'.$thumb_name);
            }
            $web_images[$i]['thumb']=$thumb_name;
        }
        ...
        return view('pages.front.webdesign', compact('menu', 'side_items', 'web', 'home', 'web_images', 'image_sizes'));
    }
}
